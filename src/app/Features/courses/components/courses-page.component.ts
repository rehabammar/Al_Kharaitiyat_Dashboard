...
export class CoursesPageComponent {
  ...
  @ViewChild(GenericTableComponent) table!: GenericTableComponent<TeacherCourse>;
 ...
 // teacher course
  ...
  onFormRowChanged(e: { field: string; value: any }) {
  ...
  onFormSaved(row: TeacherCourse) {
    this.table.patchRowById(row.teacherCoursePk, row);
  }

  onNewRow(row: TeacherCourse) {
  this.selectedCourse = row;
  this.table.prependRow(row); 
  }

  onFormRowDeleted(e: { type: 'new' | 'persisted'; id?: any; row?: any }) {
  if (e.type === 'new') {
